    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, String>> fetchedBreeds = data.map((item) {
          ...
          }.map((key, value) => MapEntry(key, value.toString())); // Cast values to String
        }).toList();
              ...
              child: Center(
                child: Column(
                  children: [
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: DataTable(
                          headingRowColor: WidgetStateColor.resolveWith(
                            (states) => const Color.fromARGB(255, 240, 245, 250), // Header background color
                          ),
                          ...
                          border: const TableBorder(
                            horizontalInside: BorderSide(
                              color: Color(0xFFF0F0F0), // Border color between rows
                              width: 1.8,
                            ),
                          ),
                        ),
                    ...
                    const SizedBox(height: 8), // Space between table and pagination
                    // Apply margin to pagination
                    Padding(
                      padding: const EdgeInsets.only(right: 20.0), // Apply right margin
                      child: _buildPaginationControls(), // Pagination directly under the table
                    ),
                  ],
                ),
              ),
      ...
      DataColumn(
        label: Text(
          ...
          style: TextStyle(
            ...
            backgroundColor: Color.fromARGB(255, 255, 255, 255), // Background color (#F0F5FA)
          ),
        ),
      ),
      DataColumn(
    ...
    child: Row(
      ...
      children: [
        IconButton(
          ...
          icon: Icon(Icons.arrow_back_ios, color: currentPage > 0 ? Colors.black54 : Colors.grey),
          iconSize: 18,
        ),
            ...
            children: [
              ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  elevation: page == currentPage ? 2 : 0, // No elevation for unselected buttons
                  backgroundColor: page == currentPage ? const Color(0xFFE1E1E6) : Colors.transparent,
                  foregroundColor: page == currentPage ? Colors.black : Colors.black54,
                  padding: EdgeInsets.zero, // Remove default padding
                  minimumSize: const Size(28, 28), // Consistent size for buttons
                  shape: page == currentPage
                      ? const CircleBorder() // Circular shape for the selected button
                      : const RoundedRectangleBorder(side: BorderSide.none), // No border or background for unselected buttons
                ),
                child: Text(
                  (page + 1).toString(),
